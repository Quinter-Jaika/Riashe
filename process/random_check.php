...
function testAllUsersBreaches($conn) {
    // Get all test users
    $users = $conn->query("SELECT id, username FROM users WHERE email LIKE '%@test.%'");
    ...
    while ($user = $users->fetch_assoc()) {
        ...
        $result = $stmt->get_result();
        $hashes = $result->fetch_assoc();
        
        if (!$hashes) {
            echo "No hash data found for user {$user['username']}\n";
            continue;
        }
        ...
        if (!$update->execute()) {
            die("Execute update failed: " . $update->error);
        }
        
        echo "User {$user['username']} - ";
        echo "SHA1: {$sha1_result['breach_count']} breaches, ";
        echo "SHA256: {$sha256_result['breach_count']} breaches, ";
        echo "MD5: {$md5_result['breach_count']} breaches\n";
...
// Run the check
testAllUsersBreaches($conn);
$conn->close();

echo "Breach check completed successfully.\n";
?>